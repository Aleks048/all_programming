...
public class Verify {
  ...
  public void randomTrees( int _noNodes, int _nTrees ) {
    ...
    for(int i=0; i<_nTrees; ++i) { 
      // Create AVL and BST trees DONE
      AVLTreeMap avlTree = new AVLTreeMap<Integer,String>();
      TreeMap binTree = new TreeMap<Integer,String>();
      // loop to insert _noNodes keys into tree
      int count = 0;
      do {
	// create random key between 0 and the maximum integer DONE
  int randKey=(int) (Math.random()*Integer.MAX_VALUE);
  
  while (avlTree.get(randKey)!=null){
    randKey=(int) (Math.random()*Integer.MAX_VALUE);  
  }
	// make sure the generated random key is not used before DONE
	if ( true ) { // Needs change !!!
	  // insert into AVL
      avlTree.put(randKey,"hi");
    // insert into BST
      binTree.put(randKey,"ho");
	  // next key
	  ++count;
	}
      } while ( count < _noNodes );
      int avlHeight = (int)(2*Math.log(avlTree.size())+2);
      
       updateCounters((int)(2*Math.log(avlTree.size())+2),height(binTree));
      //updateCounters( 0, 0); // Needs change !!! DONE
    }
    return;
  }
